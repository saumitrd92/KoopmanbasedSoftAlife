...
def loss_reconpsi(y_true, y_pred):
    global my_args
    state_features, psi_enc_inputs, enc_inputs, reconphi, reconpsi, linearx, psi_encoder = my_args
    return tf.reduce_mean(tf.square(psi_enc_inputs - reconpsi), axis=-1)

def loss_reconphi(y_true, y_pred):
    global my_args
    state_features, psi_enc_inputs, enc_inputs, reconphi, reconpsi, linearx, psi_encoder = my_args
    return tf.reduce_mean(tf.square(psi_enc_inputs - reconphi), axis=-1)

def loss_recon(y_true, y_pred):
    global my_args
    state_features, psi_enc_inputs, enc_inputs, reconphi, reconpsi, linearx, psi_encoder = my_args
    return (tf.reduce_mean(tf.square(psi_enc_inputs - reconphi), axis=-1) + tf.reduce_mean(tf.square(psi_enc_inputs - reconpsi), axis=-1))*0.5

def loss_linear(y_true, y_pred):
    global my_args
    state_features, psi_enc_inputs, enc_inputs, reconphi, reconpsi, linearx, psi_encoder = my_args
    return tf.reduce_mean(tf.square(psi_encoder(tf.reshape(y_true,[-1,state_features]) + psi_enc_inputs) - linearx), axis=-1)
...
def loss_infinity(y_true, y_pred):
    global my_args
    state_features, psi_enc_inputs, enc_inputs, reconphi, reconpsi, linearx, psi_encoder = my_args
    return tf.reduce_max(tf.abs(y_true - y_pred), axis=-1) + tf.reduce_max(tf.abs(psi_enc_inputs - reconpsi), axis=-1) + tf.reduce_max(tf.abs(psi_enc_inputs - reconphi), axis=-1)
...
def pipeline(params):
    ...
    group_features = params['group_features']
    ...
    ### Add Psi Encoder Input ###
    psi_enc_input_shape = (state_features,)
    psi_enc_inputs = Input(shape=psi_enc_input_shape, name='psi_enc_input')

    ### Add Phi Encoder ###
    phi_enc_input_shape = (group_features,)
    enc_inputs = Input(shape=phi_enc_input_shape, name='phi_enc_input')

    # ### Add Beta Network ###
    # beta_input_shape = (k,state_features+2,)
    # beta_inputs = Input(shape=beta_input_shape, name='beta_input')
    beta_dense1 = Dense(params['hp_beta_units'], activation="relu", name='beta_dense1', kernel_regularizer=L1L2(l1=params['hp_l1_reg'], l2=params['hp_l2_reg']))(enc_inputs)
    ...
    beta_dense4 = LeakyReLU(alpha=0.5)(Dense(1, name='beta_dense4')(beta_dense1))
    # beta_flatten = Flatten()(beta_dense4)

    concat_enc_inputs = Lambda(lambda z: tf.concat(z,axis=-1), name='concat')([psi_enc_inputs,beta_dense4])
    ...
    phi_encoder = Model([enc_inputs,psi_enc_inputs], phi_enc_dense3, name='phi_Encoder')

    ### Add Psi Encoder ###
    psi_enc_dense1 = Dense(params['hp_psi_enc_units'], activation="relu", name='psi_enc_dense1', kernel_regularizer=L1L2(l1=params['hp_l1_reg'], l2=params['hp_l2_reg']))(psi_enc_inputs)
    ...
    psi_encoder = Model(psi_enc_inputs, psi_enc_dense3, name='psi_Encoder')
    ...
    delta_dense3 = Dense(state_features, activation="tanh", name='delta_dense3')(delta_dense21)
    delta_net = Model(delta_inputs, delta_dense3, name='delta_net')

    ### Additional tensors ###
    reconphi = psi_decoder(phi_encoder([enc_inputs,psi_enc_inputs]))
    reconpsi = psi_decoder(psi_encoder(psi_enc_inputs))
    linearx = Koopman_Lambda(phi_encoder([enc_inputs,psi_enc_inputs]))
    predx = delta_net(Koopman_Lambda(phi_encoder([enc_inputs,psi_enc_inputs])))

    global my_args

    my_args = state_features, psi_enc_inputs, enc_inputs, reconphi, reconpsi, linearx, psi_encoder

    opt = Adam()

    ### Compile Autoencoder ###
    ae = Model([enc_inputs,psi_enc_inputs], reconphi, name='AE')
    ae.compile(loss=loss_recon, optimizer=opt)

    ### Compile Prediction Pipeline ###
    pipeline_nn = Model([enc_inputs,psi_enc_inputs], predx, name='Pipeline')
